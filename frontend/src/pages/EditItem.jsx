...
const EditItem = () => {
  ...
  return (
    <div className="flex justify-center items-center p-6 bg-[#fffcfb] min-h-screen relative">
      {/* Back Button */}
      <div className="absolute top-6 left-6 cursor-pointer hover:scale-110 transition-transform" onClick={() => navigate(-1)}>
      ...
      <div className="w-full max-w-lg bg-white shadow-[0_8px_30px_rgb(0,0,0,0.08)] rounded-[2.5rem] p-8 md:p-10 border border-gray-50 mt-10">
        ...
        <form className="flex flex-col gap-5" onSubmit={handleSubmit}>
          ...
          <div className="flex flex-col gap-1.5">
            ...
            <input required type="text" className="w-full p-3.5 rounded-xl border border-gray-200 outline-none focus:border-[#ff4d2d]" onChange={(e) => setName(e.target.value)} value={name} />
          </div>

          {/* File Input */}
          <div className="flex flex-col gap-1.5">
            <label className="text-sm font-bold text-gray-600 ml-1">Food Image</label>
            <input type="file" accept="image/*" onChange={handleImageChange} className="w-full p-2 text-sm text-gray-500 file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-sm file:font-semibold file:bg-[#ff4d2d]/10 file:text-[#ff4d2d] border border-gray-200 rounded-xl" />
          </div>
          ...
          <div className="flex gap-4">
            <div className="flex-1 flex flex-col gap-1.5">
              ...
              <input required type="number" className="w-full p-3.5 rounded-xl border border-gray-200 outline-none focus:border-[#ff4d2d]" onChange={(e) => setPrice(e.target.value)} value={price} />
            </div>
            <div className="flex-1 flex flex-col gap-1.5">
              <label className="text-sm font-bold text-gray-600 ml-1">Food Type</label>
              <select className="w-full p-3.5 rounded-xl border border-gray-200 outline-none focus:border-[#ff4d2d]" value={foodType} onChange={(e) => setFoodType(e.target.value)}>
          ...
          <div className="flex flex-col gap-1.5">
            ...
            <select className="w-full p-3.5 rounded-xl border border-gray-200 outline-none focus:border-[#ff4d2d]" value={category} onChange={(e) => setCategory(e.target.value)}>
              {categories.map(cat => <option key={cat} value={cat}>{cat}</option>)}
            </select>
          </div>

          <button disabled={loading} type="submit" className="w-full bg-[#ff4d2d] text-white font-bold py-4 rounded-xl mt-2 shadow-lg hover:bg-[#e64427] transition-all disabled:opacity-50">
